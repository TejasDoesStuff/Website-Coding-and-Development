...
const JobCard = ({ post }: { post: Post }) => {
  ...
  for (let i = 0; i < post.requirements.length; i++) {
    requirements.push(<li key={i}>• {post.requirements[i]}</li>);
  }

  const tags = [];
  for (let i = 0; i < post.tags.length; i++) {
    tags.push(
      <Badge key={i} variant="secondary" className="text-md mx-0.5">
        {post.tags[i]}
      </Badge>
    );
  }
    ...
    <div>
      <Card>
        <CardHeader className="text-left">
          ...
          <Link href={link} className="text-2xl hover:underline text-left">
            {post.title}
          </Link>
          {/* </Button> */}
